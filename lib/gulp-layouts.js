...
function transfromData(datasets, defaultData) {
  let adInfo = require('../sources/ads.json');
  ...
  // Add ad information, which is based on the group
  _.each(datasets, (data, di) => {
    datasets[di] = _.map(data, d => {
      let groupId =
        d.dataset === 'groups'
          ? d.id
          : d.dataset === 'lists' && d.groups && d.groups[0]
            ? d.groups[0].id
            : d.dataset === 'items' && d.groups && d.groups[0]
              ? d.groups[0].id
              : d.dataset === 'items' &&
                d.lists &&
                d.lists[0] &&
                d.lists[0].groups &&
                d.lists[0].groups[0]
                ? d.lists[0].groups[0].id
                : undefined;

      if (groupId && adInfo[groupId]) {
        d.ad = _.clone(adInfo[groupId]);
      }
  ...
  datasets.index = [
    {
      ...
      metaTitle: defaultData.content.metaTitle,
      twitterShare:
        defaultData.content.twitterShare || defaultData.content.metaTitle,
      twitterTitle:
        defaultData.content.twitterTitle || defaultData.content.metaTitle,
      facebookTitle:
        defaultData.content.facebookTitle || defaultData.content.metaTitle,
      groups: datasets.groups,
      siteSettings: defaultData.content,
      baseURL: defaultData.config.publish.production.url,
      mainVisual: 'assets/images/responsive/social-share.jpg',
      twitterImage: 'assets/images/responsive/social-share-twitter.jpg',
      twitterImageWidth: 920,
      twitterImageHeight: 458,
      facebookImage: 'assets/images/responsive/social-share-facebook.jpg',
      facebookImageWidth: 1200,
      facebookImageHeight: 630
    }
  ];
...
function renderer(gulp, options = {}) {
  return () => {
    ...
    let defaultData = {
      ...
      jsString: (input = '', singleQuote = true) => {
        return _.isString(input)
          ? input
            .replace(/(<([^>]+)>)/gi, '')
            .replace(/\r?\n|\r/g, ' ')
            .replace(!singleQuote ? /"/g : /'/g, !singleQuote ? '\\"' : '\\\'')
            .trim()
          : '';
      }
    };
      ...
      _.each(data, d => {
        ...
        tasks.push(
          file(d.filename || di + '/' + _.snakeCase(d.id) + '.html', layout, {
            ...
            // Apparenlty rel=noopener breaks native handling of links
            // on mobile devices.  :(
            //.pipe(noopener.warn())
...
// Export
renderer.parseAirtableContent = parseAirtableContent;
